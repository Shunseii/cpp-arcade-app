...
bool Triangle::ContainsPoint(const Vec2D& p) const {
	...
	// Calculate area of three subtriangles 
	float a1 = Area(p, GetP1(), GetP2());
	float a2 = Area(GetP0(), p, GetP2());
	float a3 = Area(GetP0(), GetP1(), p);
	
	// If the sum of the area of the sub trianges is equal to the area of the triangle
	//   then the point is contained within the triangle
	return IsEqual(a1 + a2 + a3, thisArea);
}
