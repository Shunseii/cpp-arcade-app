...
void BreakoutGameLevel::CreateDefaultLevel(const AARectangle& boundary) {
	...
	for (int r = 0; r < NUM_BLOCK_ROWS; ++r) {
		AARectangle blockRect = {
			...
			static_cast<unsigned int>(BLOCK_WIDTH),
			static_cast<unsigned int>(BLOCK_HEIGHT),
		};
...
LayoutBlock FindLayoutBlockForSymbol(
...
std::vector<BreakoutGameLevel> BreakoutGameLevel::LoadLevelsFromFile(
	...
	levelCommand.parseFunc = [&](ParseFuncParams params) {
		...
		level.Init(AARectangle(Vec2D::ZERO, App::Singleton().GetWidth(), App::Singleton().GetHeight()));
	...
	layoutCommand.parseFunc = [&](ParseFuncParams params) {
		...
		int screenWidth = App::Singleton().GetWidth();
		...
		for (unsigned int c = 0; c < params.line.length(); ++c) {
			if (params.line[c] != '-') {
				LayoutBlock layoutBlock = FindLayoutBlockForSymbol(
						layoutBlocks, params.line[c]);
